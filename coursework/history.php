...
<div class="note">
*Click the Package ID to VIEW Package
</div> 
</br></br> 
...
WHERE Paid='YES' AND package.id = (SELECT id FROM user WHERE username ='$username1')";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<tr><td>" ."<clickable><a href='hdetail.php?package=$package1'>".$package1."</a>". "</clickable>". "</td><td>" . $row["type"]. "</td><td>" . $row["date"]. "</td><td>" .$row["time"]. "</td><td>" .$row["Quantity"]. "</td><td>" .$row["Price1"]. "</td></tr>"; 
...
/*$sql = "SELECT ROUND (SUM(package.Quantity*package.Price),2) AS grandtotal
FROM package INNER JOIN cart ON package.id = cart.id 
WHERE Paid=0 AND package.id = (SELECT id FROM user WHERE username ='$username1') ";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr><td colspan='6'>"."<big>GRAND TOTAL</big>"."</td><td>"."<big>".$row["grandtotal"]."</big>"."</td></tr>";
